...
<div class="dashboard-wrapper" x-data="gridDashboardController">
    ...
    <div class="dashboard-main">
        ...
        <!-- === SECTION D: Grid Visualization (Dynamic Price Ladder) === -->
        <template x-if="gridLevels && gridLevels.length > 0">
            <section aria-label="Visualização do Grid">
                <template x-for="(gridData, gIdx) in gridLevels" :key="gIdx">
                    <div class="dash-card">
                        <div class="card-header-custom">
                            <h6>
                                ...
                                <span class="badge bg-info ms-1" style="font-size: 0.65rem;" x-text="gridData.grid.symbol"></span>
                            </h6>
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge" style="font-size: 0.6rem;"
                                    :class="gridData.grid.status === 'active' ? 'bg-success' : 'bg-secondary'"
                                    x-text="gridData.grid.status === 'active' ? 'Active' : gridData.grid.status.charAt(0).toUpperCase() + gridData.grid.status.slice(1)">
                                </span>
                                <span class="badge bg-warning text-dark" style="font-size: 0.6rem;">
                                    <i class="bi bi-broadcast"></i> Live
                                </span>
                                <span class="badge bg-outline-dim" style="font-size: 0.6rem;"
                                    x-text="gridData.total_open + ' abertas'">
                                </span>
                            </div>
                        </div>
                        <div class="card-body-custom p-0">
                            <div class="grid-ladder custom-scroll" style="max-height: 500px;">
                                ...
                                <template x-for="(level, sIdx) in gridData.sell_levels" :key="'s' + sIdx">
                                    <div class="ladder-level level-sell ladder-level-animated">
                                        <span class="level-badge badge-sell">
                                            <i class="bi bi-arrow-up-short"></i><span x-text="'S' + (gridData.sell_levels.length - sIdx)"></span>
                                        </span>
                                        <span class="level-price text-sell" x-text="formatPrice(level.price)"></span>
                                        <span class="level-qty d-none d-sm-inline" x-text="formatBTC(level.quantity) + ' un'"></span>
                                        <span class="level-qty d-none d-md-inline" x-text="'~' + formatUSD(level.price * level.quantity)"></span>
                                        <span class="level-status badge-status"
                                            :class="getStatusBadgeClass(level.status)"
                                            x-text="getStatusLabel(level.status)"></span>
                                    </div>
                                </template>

                                <!-- Current Price Marker (dynamic) -->
                                <div class="ladder-level level-current-price ladder-price-animated"
                                    :class="{ 'price-flash-up': priceDirection === 'up', 'price-flash-down': priceDirection === 'down' }">
                                    <span class="level-badge" style="background: var(--dash-info-bg); color: var(--dash-info);">
                                        <i class="bi bi-cursor"></i>
                                    </span>
                                    <span class="level-price" style="color: var(--dash-info); font-weight: 700;"
                                        x-text="currentPrice > 0 ? formatPrice(currentPrice) : '--'"></span>
                                ...
                                <template x-for="(level, bIdx) in gridData.buy_levels" :key="'b' + bIdx">
                                    <div class="ladder-level level-buy ladder-level-animated">
                                        <span class="level-badge badge-buy">
                                            <i class="bi bi-arrow-down-short"></i><span x-text="'B' + (bIdx + 1)"></span>
                                        </span>
                                        <span class="level-price text-buy" x-text="formatPrice(level.price)"></span>
                                        <span class="level-qty d-none d-sm-inline" x-text="formatBTC(level.quantity) + ' un'"></span>
                                        <span class="level-qty d-none d-md-inline" x-text="'~' + formatUSD(level.price * level.quantity)"></span>
                                        <span class="level-status badge-status"
                                            :class="getStatusBadgeClass(level.status)"
                                ...
                                <!-- Empty state -->
                                <template x-if="gridData.sell_levels.length === 0 && gridData.buy_levels.length === 0">
                                    <div class="text-center text-dim py-4">
                                        <i class="bi bi-inbox" style="font-size: 1.5rem;"></i>
                                        <p class="mb-0 mt-1" style="font-size: 0.8rem;">Nenhuma ordem aberta</p>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="d-flex flex-wrap gap-3 px-3 py-2" style="background: var(--dash-card-border); font-size: 0.75rem;">
                                <span class="text-dim"><i class="bi bi-graph-up"></i> Spacing: <span x-text="(gridData.grid.grid_spacing_percent * 100).toFixed(1) + '%'"></span></span>
                                <span class="text-dim"><i class="bi bi-layers"></i> <span x-text="gridData.grid.grid_levels"></span> níveis</span>
                                <span class="text-dim"><i class="bi bi-cash"></i> <span x-text="formatUSD(gridData.grid.capital_allocated_usdc)"></span> total</span>
                                <span class="text-dim ms-auto"><i class="bi bi-clock"></i> <span x-text="lastUpdateFormatted"></span></span>
                            </div>
                        </div>
                    </div>
                </template>
        ...
        <!-- Fallback: No grid levels available -->
        <template x-if="!gridLevels || gridLevels.length === 0">
            <section aria-label="Visualização do Grid">
                <div class="dash-card">
                    <div class="card-header-custom">
                    ...
                    <div class="card-body-custom text-center text-dim py-4">
                        <i class="bi bi-grid-3x3-gap" style="font-size: 2rem; opacity: 0.4;"></i>
                        <p class="mb-0 mt-2" style="font-size: 0.85rem;">Nenhum grid ativo no momento</p>
                    </div>
                </div>
            </section>
        </template>
